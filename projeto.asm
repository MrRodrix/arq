...
loop:
    ...
    blt $t1, '[', to_lower                           #se for Z ate A deixa minusculo
    # a - z
    blt $t1, 'a', next_step                          #se for simbolo qualquer de [ ate '
...
end_verify:
    ...
    addi $t2, $t2, 1                      # t2++
    lb $t4, buffer($t2)                   #repete o processo para o resto das letras em 'endfile'
    bne $t4, 'n',loop 
    addi $t2, $t2, 1
    lb $t4, buffer($t2)                
    bne $t4, 'd',loop
    ...
    addi $t2, $t2, 1
    lb $t4, buffer($t2)
    beq $t4, 0, reverse                   #chegando no fim vai pro reverse reverter a ordem
    beq $t4, 10, reverse
    j loop
...
end_verify_reverse:
    ...
    addi $t2, $t2, 1
    lb $t4, buffer($t2)
    beq $t4, 0, fim                          #chegando no fim vai para fim finalizar o programa
    beq $t4, 10, fim
    j reverse_str
...
clear_i:
    li $t3, 0                                 #zerando o auxiliar
    jr $ra

clear_t0:
    li $t0, 0                                 #zerando o indice
    jr $ra
